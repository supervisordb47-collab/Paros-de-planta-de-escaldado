(() => {
  ...
  function normalizeStateRecords(records = []) {
    ...
    records.forEach(rec => {
      ...
      const key = isShiftReportRecord(item)
        ? `SHIFT|${recordDateKey(item)}|${safe(item.shift)}`
        : item.fingerprint;
      ...
      const itemStamp = String(item.updatedAt || item.createdAt || '');
      if (itemStamp >= prevStamp) byKey.set(key, item);
    });
    return [...byKey.values()];
  }
  ...
  function normalizeShiftReports(reports = []) {
    ...
    reports.forEach(rep => {
      ...
      const key = `${safe(item.date)}|${safe(item.shift)}`;
      const prev = byKey.get(key);
      if (!prev) {
        byKey.set(key, item);
        return;
  ...
  function parseSiloLine(line) {
    ...
    let finalized = false;
    if (/finaliz|terminad|cerrad/.test(afterClean)) {
      status = 'Finalizado';
      finalized = true;
    } else if (afterClean.includes('vac')) {
      status = 'Vacío';
    } else if (afterClean.includes('lleno') || afterClean.includes('completo')) {
      status = 'Lleno';
    } else if (afterClean.includes('proceso') || afterClean.includes('carg') || afterClean.includes('descarg') || afterClean.includes('trabaj') || afterClean.includes('oper')) {
      status = 'En proceso';
    }

    const note = afterColon
      .replace(/^\d{1,4}(?:[.,]\d+)?\s*qq?\.?\s*/i, '')
      .replace(/\b(finalizad[oa]?|terminad[oa]?|cerrad[oa]?|vac[ií]o|lleno|completo|en proceso|cargando|descargando|trabajando|operando)\b/ig, '')
      .replace(/[\s·|]+$/g, '')
      .trim();
    ...
    return {
      ...
      note: note || (finalized ? 'Finalizado' : qq === 0 ? 'Vacío' : ''),
      status,
      finalized,
      raw
    };
  ...
  function siloStatusClass(status = '') {
    const normalized = stripAccents(status).toLowerCase();
    if (normalized.includes('finaliz')) return 'danger';
    if (normalized.includes('lleno')) return 'danger';
    if (normalized.includes('vac')) return 'good';
    if (normalized.includes('proceso')) return 'warn';
    return 'good';
  }

  function normalizeSiloState(info = {}, silo = 0) {
    const capacity = SILO_CAPACITY[silo] || toNumber(info.capacity, 0);
    const qq = Math.max(0, toNumber(info.qq, 0));
    ...
    const rawStatus = safe(info.status || info.state || info.note || '');
    const finalized = Boolean(info.finalized) || /finaliz|terminad|cerrad/.test(stripAccents(rawStatus).toLowerCase());
    const status = finalized
      ? 'Finalizado'
      : rawStatus || (qq === 0 ? 'Vacío' : qq >= capacity ? 'Lleno' : 'En proceso');
    const note = safe(info.note || info.raw || (finalized ? 'Finalizado' : qq === 0 ? 'Vacío' : ''));
    return {
      silo,
      qq,
      capacity,
  ...
  function reportDigestFromData(report = {}) {
    const silosText = Object.values(report.silos || {})
      .map(s => {
        const normalized = normalizeSiloState(s, s.silo);
        return `Silo ${normalized.silo}: ${normalized.qq} qq · ${normalized.status}${normalized.note ? ` · ${normalized.note}` : ''}`;
      })
      .join(' | ');
    const dryersText = Object.values(report.dryers || {})
      .map(d => `Secadora ${d.dryer}: ${safe(d.status || d.state || 'Sin detalle')}`)
      .join(' | ');
    return { silosText, dryersText };
  }

  function rebuildShiftReportText(report = {}) {
    const silos = Object.values(report.silos || {})
      .map(s => normalizeSiloState(s, s.silo))
      .sort((a, b) => a.silo - b.silo);
    const dryers = Object.values(report.dryers || {})
      .map(d => ({ dryer: d.dryer, status: safe(d.status || d.state || 'Sin detalle') }))
      .sort((a, b) => Number(a.dryer) - Number(b.dryer));
    const casulla = Array.isArray(report.casulla) ? report.casulla : [];
    const remojo = Array.isArray(report.remojo) ? report.remojo : [];
    const notes = Array.isArray(report.notes) ? report.notes : [];
    const lines = [];
    lines.push(`FECHA: ${report.date || todayISO()}`);
    lines.push(`TURNO: ${report.shift || 'Día'}`);
    lines.push(`TOTAL SECADAS: ${toNumber(report.totalSecadas)}`);
    lines.push('');
    lines.push('SECADORAS:');
    dryers.forEach(d => lines.push(`Secadora ${d.dryer}: ${d.status}`));
    lines.push('');
    lines.push('SILOS DE GRANZA:');
    silos.forEach(s => {
      const extra = s.finalized ? ' finalizado' : '';
      lines.push(`Silo ${s.silo}: ${s.qq} qq · ${s.status}${extra}${s.note ? ` · ${s.note}` : ''}`);
    });
    if (casulla.length) {
      lines.push('');
      lines.push('CASULLA:');
      casulla.forEach(x => lines.push(x));
    }
    if (remojo.length) {
      lines.push('');
      lines.push('SILOS DE REMOJO:');
      remojo.forEach(x => lines.push(x));
    }
    if (notes.length) {
      lines.push('');
      lines.push('OBSERVACIONES:');
      notes.forEach(x => lines.push(x));
    }
    return lines.join('\n').trim();
  }

  function updateShiftReportSilo(reportId, siloNo, patch = {}) {
    const idx = state.records.findIndex(r => r.id === reportId && isShiftReportRecord(r));
    if (idx < 0) return false;
    const rec = clone(state.records[idx]);
    rec.reportData = rec.reportData || {};
    rec.reportData.silos = rec.reportData.silos || {};
    rec.reportData.dryers = rec.reportData.dryers || {};
    rec.reportData.casulla = Array.isArray(rec.reportData.casulla) ? rec.reportData.casulla : [];
    rec.reportData.remojo = Array.isArray(rec.reportData.remojo) ? rec.reportData.remojo : [];
    rec.reportData.notes = Array.isArray(rec.reportData.notes) ? rec.reportData.notes : [];
    const current = rec.reportData.silos[String(siloNo)] || { silo: siloNo };
    const merged = normalizeSiloState({ ...current, ...patch }, siloNo);
    rec.reportData.silos[String(siloNo)] = merged;
    rec.reportData.notesText = [...(rec.reportData.casulla || []), ...(rec.reportData.remojo || []), ...(rec.reportData.notes || [])].join(' · ').trim();
    rec.reportText = rebuildShiftReportText(rec.reportData);
    const digest = reportDigestFromData(rec.reportData);
    rec.reportSilosText = digest.silosText;
    rec.reportDryersText = digest.dryersText;
    rec.mainStop = rec.reportData.notesText || rec.mainStop;
    rec.notes = rec.reportData.notesText || rec.notes;
    ...
    renderAll();
    showToast('Silo actualizado', `Silo ${siloNo} guardado y sincronizado en el mismo registro.`);
    return true;
  }
  ...
  function buildShiftReportRecord(report, rawText) {
    ...
    const digest = reportDigestFromData(report);
    const rec = {
      id: uid('rec'),
      user: currentUser()?.username || 'ADMIN',
      fullName: currentUser()?.fullName || currentUser()?.username || 'ADMIN',
      ...
      reportData: {
        ...clone(report),
        silos: Object.fromEntries(Object.entries(report.silos || {}).map(([k, v]) => [k, normalizeSiloState(v, Number(k))]))
      },
      reportSilosText: digest.silosText,
      reportDryersText: digest.dryersText
    };
    rec.fingerprint = makeRecordFingerprint(rec);
    return rec;
  }
  ...
  function buildReportPreview(report) {
    ...
    const silos = Object.keys(report?.silos || {}).length ? Object.values(report.silos).map(s => {
      ...
      const status = s.finalized ? 'Finalizado' : (s.status || s.state || 'Sin estado');
      return `<div class="report-line"><strong>Silo ${escapeHtml(s.silo)}</strong> · ${escapeHtml(String(s.qq))} qq · ${fill}% · ${escapeHtml(status)}${s.note ? ` · ${escapeHtml(s.note)}` : ''}</div>`;
    ...
    return `
      <div class="report-banner">
        ...
        <div class="report-sub">Cada secada equivale a ${escapeHtml(String(SECADA_QQ))} qq. El sistema guarda una sola masa por fecha y turno.</div>
      </div>
  ...
  function renderSiloInsights() {
    ...
    const activeReportId = data.report?.id || null;
    const silosHtml = data.silos.map(s => {
      const statusClass = siloStatusClass(s.status);
      const currentStatus = s.finalized ? 'Finalizado' : s.status;
      return `
      <div class="silo-card ${escapeHtml(statusClass)}">
        <header>
          <div>
          ...
          <div class="silo-badge ${escapeHtml(s.badge)}">${escapeHtml(currentStatus)} · ${escapeHtml(String(Math.round(s.availability)))}%</div>
        </header>
        <div class="silo-body">
          <div class="silo-tank ${escapeHtml(statusClass)}" aria-hidden="true">
            ...
            <div class="silo-shell">
              ...
              <div class="silo-ribs"></div>
              <div class="silo-tank-label">${escapeHtml(String(s.no))}</div>
            </div>
            <div class="silo-legs"><span></span><span></span></div>
          </div>
          <div class="silo-detail">
            ...
            <div class="silo-editor">
              <label class="field compact">
                <span>qq actuales</span>
                <input data-silo-qq="${escapeHtml(String(s.no))}" type="number" min="0" step="1" value="${escapeHtml(String(s.qq))}" />
              </label>
              <label class="field compact">
                <span>Estado</span>
                <select data-silo-status="${escapeHtml(String(s.no))}">
                  ${['En proceso','Finalizado','Vacío','Lleno'].map(opt => `<option value="${escapeHtml(opt)}" ${opt === currentStatus ? 'selected' : ''}>${escapeHtml(opt)}</option>`).join('')}
                </select>
              </label>
              <label class="field compact wide">
                <span>Nota</span>
                <input data-silo-note="${escapeHtml(String(s.no))}" type="text" value="${escapeHtml(s.note || '')}" placeholder="Observación del silo" />
              </label>
              <button class="btn soft silo-save" data-silo-save="${escapeHtml(String(s.no))}" data-report-id="${escapeHtml(String(activeReportId || ''))}">Guardar silo</button>
            </div>
    ...
    box.querySelectorAll('[data-silo-save]').forEach(btn => {
      btn.addEventListener('click', () => {
        const siloNo = Number(btn.dataset.siloSave);
        const reportId = btn.dataset.reportId;
        const card = btn.closest('.silo-card');
        if (!card || !reportId) return;
        const qqEl = card.querySelector(`[data-silo-qq="${siloNo}"]`);
        const statusEl = card.querySelector(`[data-silo-status="${siloNo}"]`);
        const noteEl = card.querySelector(`[data-silo-note="${siloNo}"]`);
        updateShiftReportSilo(reportId, siloNo, {
          qq: qqEl ? toNumber(qqEl.value, 0) : 0,
          status: statusEl ? statusEl.value : 'En proceso',
  ...
  function weekSummaryForOffset(offsetWeeks = 0, records = getAnalyticsRecords(), baseDate = dashboardAnchorDate()) {
  ...
  function getAnalyticsRecords(records = getRecordsVisible()) {
    const list = Array.isArray(records) ? records.slice() : [];
    const reportKeys = new Set(list.filter(isShiftReportRecord).map(r => `${recordDateKey(r)}|${String(r.shift || '')}`));
    return list.filter(r => {
      if (isShiftReportRecord(r)) return true;
      const key = `${recordDateKey(r)}|${String(r.shift || '')}`;
      return !reportKeys.has(key);
    });
  }
  ...
  function dashboardMonthRecords(records = getAnalyticsRecords()) {
  ...
  function dashboardSelectedWeekRecords(records = getAnalyticsRecords()) {
    const weekKey = dashboardSelectedWeekKey();
    if (!weekKey) return [];
    return completedRecords(records).filter(r => isoWeekKey((r.date || '').slice(0, 10)) === weekKey);
  }

  function monthWeeksForCurrentSelection(records = getAnalyticsRecords()) {
    const monthKey = dashboardMonthKey();
  ...
  function dashboardRecords(records = getAnalyticsRecords()) {
    const { year, month } = dashboardFilterState();
    return records.filter(r => {
      const dateKey = (r.date || '').slice(0, 10);
      if (!dateKey) return false;
  ...
  function completedRecords(records = getAnalyticsRecords()) {
    return records.filter(r => toNumber(r.secadas) > 0);
  }

  function todayWeekMonthStats(records = getAnalyticsRecords()) {
  ...
  function buildMissingDryerList(records = getAnalyticsRecords()) {
    const total = dryerCount();
    const map = new Map();
    records.forEach(r => {
      const dateKey = (r.date || '').slice(0, 10);
