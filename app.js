(() => {
  ...
  function dryerConsolidatedData(records = dashboardRecords()) {
    ...
    const currentWeek = weekSummaryForOffset(0, records, dashboardAnchorDate()).weekRecords;
  ...
  function dashboardFilterState() {
    ...
    return {
      ...
      week: $('dashWeekFilter')?.value || String(isoWeekNumber(today)).padStart(2, '0')
    };
  }
  ...
  function dashboardAnchorDate() {
    const { year, month, week } = dashboardFilterState();
    const yr = Number(year || todayISO().slice(0, 4));
    if (week) {
      const wk = Math.min(53, Math.max(1, parseIntMaybe(week) || isoWeekNumber(todayISO())));
      return isoWeekStartOfYearWeek(yr, wk);
    }
  ...
  function dashboardRecords(records = getRecordsVisible()) {
    ...
    return records.filter(r => {
      ...
      if (week) {
        const weekKey = `${String(year || dateKey.slice(0, 4))}-W${String(week).padStart(2, '0')}`;
        if (isoWeekKey(dateKey) !== weekKey) return false;
        return true;
      }
  ...
  function recordSummary() {
    ...
    const allRecs = getRecordsVisible();
    const { year, month } = dashboardFilterState();
    const monthRecords = allRecs.filter(r => {
      const dateKey = (r.date || '').slice(0, 10);
      if (!dateKey) return false;
      if (year && dateKey.slice(0, 4) !== String(year)) return false;
      if (month && dateKey.slice(5, 7) !== String(month).padStart(2, '0')) return false;
      return true;
    });
  ...
  function todayWeekMonthStats(records = dashboardRecords()) {
    const completed = completedRecords(records);
    const today = todayISO();
    const weekStart = weekStartKey(dashboardAnchorDate());
    const weekEnd = (() => {
    ...
    return {
      ...
      month: sum(completed.filter(r => (r.date || '').slice(0, 7) === monthKey).map(r => toNumber(r.secadas)))
    };
  }
  ...
  function populatePeriodFilters() {
    ...
    if (dashWeekSel) {
      const current = dashWeekSel.value || currentWeek;
      dashWeekSel.innerHTML = Array.from({ length: 53 }, (_, i) => `<option value="${String(i + 1).padStart(2, '0')}">Semana ${String(i + 1).padStart(2, '0')}</option>`).join('');
      dashWeekSel.value = current;
    }
    if (dashYearSel) {
      const current = dashYearSel.value || currentYear;
      dashYearSel.innerHTML = years.map(y => `<option value="${escapeHtml(y)}">${escapeHtml(y)}</option>`).join('');
  ...
  function renderDashboardHistory() {
    ...
    const recs = dashboardRecords().filter(r => {
      const dateKey = (r.date || '').slice(0, 10);
      if (year && dateKey.slice(0, 4) !== year) return false;
      if (month && dateKey.slice(0, 7) !== month) return false;
      if (week && isoWeekKey(dateKey) !== week) return false;
...
function renderAll() {
  ...
  function bindControls() {
    ...
    ['dashMonthFilter', 'dashWeekFilter'].forEach(id => {
      const el = $(id);
