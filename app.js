(() => {
  ...
  function makeRecordFingerprint(rec) {
    ...
    if (secadas > 0) {
      return [
        'COMP',
        rec.date || '', rec.dryer || '', rec.loadAt || '', rec.unloadAt || '',
        rec.siloLoad || '', rec.siloOut || ''
      ].map(v => safe(v).toUpperCase()).join('|');
    }
    return [
      'STOP',
      rec.date || '', rec.dryer || '', String(rec.stopHours ?? 12), safe(rec.stopType || 'programado').toUpperCase()
    ].join('|');
  }
  ...
  function bulkPreview() {
    ...
    const dryerLines = Array.from({ length: dryers }, (_, i) => i + 1).map(d => {
      ...
      const missingLabel = info.secadas > 0 ? '' : ' · Paro automático 12 h';
      return `Secadora ${escapeHtml(String(d))} = <strong>${info.secadas}</strong> secadas · Promedio: <strong>${escapeHtml(formatMinutes(avgMins))}</strong> · Paros: <strong>${info.paros}</strong>${missingLabel}`;
    });
    box.innerHTML = `
      <strong>Vista previa consolidada</strong><br>
      ${dryerLines.join('<br>') || 'Sin datos por secadora.'}<br>
      Total secadas: <strong>${totalSecadas}</strong> · Paros sugeridos: <strong>${missingTotals.length}</strong> · Registros a guardar: <strong>${rows.length}</strong><br>
      Días con datos: <strong>${byDate.size}</strong> · Paros / pendientes: <strong>${totalStops}</strong>
  ...
  function renderCharts() {
    ...
    drawLineChart($('forecastChart'), weeklyForecastSeries().labels, weeklyForecastSeries().values, '#8b5cf6');
